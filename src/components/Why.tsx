...
const WhyUs: React.FC = () => {
  ...
  const getImageStyle = (position: string) => {
    ...
    const currentX = centerX + (endPos.x - centerX) * scrollProgress * scaleFactor;
    const currentY = centerY + (endPos.y - centerY) * scrollProgress * scaleFactor;
  ...
  return (
    ...
    >
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Butler:wght@400;500;600;700&family=Anton&display=swap');
      ...
      >
        <div className="absolute inset-0 overflow-hidden pointer-events-none">
          <div className="absolute top-1/4 left-1/4 w-48 md:w-96 h-48 md:h-96 bg-emerald-500/10 rounded-full blur-3xl"></div>
          <div className="absolute bottom-1/4 right-1/4 w-40 md:w-80 h-40 md:h-80 bg-teal-600/10 rounded-full blur-3xl"></div>
        </div>
        ...
        <div className="cta-wrapper relative w-full h-full">
          <div className="container mx-auto h-full relative">
            <div 
              ref={contentRef}
              className="cta-contant-wrap absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 text-center z-20 max-w-4xl px-4 md:px-8 transition-opacity duration-700 w-full"
              style={{ opacity: contentOpacity }}
            >
              <div className="vibe-title mb-6 md:mb-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-6 mb-6 md:mb-8">
                {benefits.map((benefit, index) => (
                  <div 
                    key={index}
                    className="benefit-card bg-white/5 backdrop-blur-md rounded-lg p-3 md:p-6 border border-lime-400/20 transition-all duration-300 hover:border-lime-400 hover:bg-white/15 hover:shadow-lg hover:shadow-lime-400/30 cursor-pointer group"
                    style={{
                  ...
                  >
                    <p 
                      className="text-white text-sm md:text-lg font-bold uppercase tracking-wide transition-all duration-300 group-hover:text-lime-400"
                      style={{ fontFamily: "'Anton', sans-serif" }}
                    >
                      {benefit}
                    </p>
      ...
      {/* Floating Images Section - Hidden on Mobile */}
      <div className="absolute left-0 w-full pointer-events-none hidden md:block" style={{ top: '100vh', zIndex: 30 }}>
